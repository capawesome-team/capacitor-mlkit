# Capacitor ML Kit GenAI Rewriting Plugin

Unofficial Capacitor plugin for [ML Kit GenAI Rewriting](https://developers.google.com/ml-kit/genai/rewriting).[^1]
...
The GenAI Rewriting plugin is typically used to reword short messages in different tones and styles on-device using Gemini Nano, for example:

- **Tone adjustments**: Rewrite a message to sound more friendly or more professional before sending it.
- **Length adjustments**: Shorten or elaborate a draft to fit the situation.
- **Expressive messaging**: Add emojis or rephrase text to give it a fresh look.
...
```
Use the `capacitor-plugins` skill from `capawesome-team/skills` to install the `@capacitor-mlkit/genai-rewriting` plugin in my project.
```
...
- `$mlkitGenaiRewritingVersion` version of `com.google.mlkit:genai-rewriting` (default: `1.0.0-beta1`)

## Usage

The following example shows how to check the feature status, download the feature and rewrite a text.

### Check the feature status

Before using the rewriting feature, check whether it is available on the device. The feature availability depends on the configured output type and language:
...
const checkFeatureStatus = async () => {
  const { featureStatus } = await GenAiRewriting.checkFeatureStatus({
    language: Language.English,
    outputType: OutputType.Elaborate,
  });
...
```typescript
import { GenAiRewriting, Language, OutputType } from '@capacitor-mlkit/genai-rewriting';

const downloadFeature = async () => {
  await GenAiRewriting.addListener('downloadProgress', event => {
    console.log('Total bytes downloaded:', event.totalBytesDownloaded);
  });
  await GenAiRewriting.downloadFeature({
    language: Language.English,
    outputType: OutputType.Elaborate,
  });
};
...
Pass the text to `rewrite(...)` to generate the rewritten suggestions. Add a listener for the `inferenceProgress` event to receive partial results as they are generated:

```typescript
import { GenAiRewriting, Language, OutputType } from '@capacitor-mlkit/genai-rewriting';

const rewrite = async () => {
  await GenAiRewriting.addListener('inferenceProgress', event => {
    console.log('Partial result:', event.text);
  });
  const { results } = await GenAiRewriting.rewrite({
    text: 'Capacitor is an open source native runtime for building Web Native apps.',
    language: Language.English,
    outputType: OutputType.Elaborate,
  });
  return results;
...
| Param         | Type                                                      |
| ------------- | --------------------------------------------------------- |
| **`options`** | <code><a href="#rewriteoptions">RewriteOptions</a></code> |

**Returns:** <code>Promise&lt;<a href="#rewriteresult">RewriteResult</a>&gt;</code>
...
| ------------------- | ------------------------------------------------------- | --------------------------------------------------------- | ----- |
| **`featureStatus`** | <code><a href="#featurestatus">FeatureStatus</a></code> | The current availability status of the rewriting feature. | 8.2.0 |
...
| **`outputType`** | <code><a href="#outputtype">OutputType</a></code> | The style in which the input text should be rewritten. | <code>OutputType.Elaborate</code> | 8.2.0 |
...
| Prop          | Type                  | Description                                                               | Since |
| ------------- | --------------------- | ------------------------------------------------------------------------- | ----- |
| **`results`** | <code>string[]</code> | The rewritten text suggestions, sorted in descending order of confidence. | 8.2.0 |
...
| Prop       | Type                | Description                                              | Since |
| ---------- | ------------------- | -------------------------------------------------------- | ----- |
| **`text`** | <code>string</code> | The text to rewrite. The input must be under 256 tokens. | 8.2.0 |
...
| Members            | Value                       | Description                                    | Since |
| ------------------ | --------------------------- | ---------------------------------------------- | ----- |
| **`Elaborate`**    | <code>'ELABORATE'</code>    | Elaborate the input text.                      | 8.2.0 |
| **`Emojify`**      | <code>'EMOJIFY'</code>      | Insert appropriate emojis into the input text. | 8.2.0 |
| **`Shorten`**      | <code>'SHORTEN'</code>      | Shorten the input text.                        | 8.2.0 |
| **`Friendly`**     | <code>'FRIENDLY'</code>     | Make the input text more friendly.             | 8.2.0 |
| **`Professional`** | <code>'PROFESSIONAL'</code> | Make the input text more professional.         | 8.2.0 |
| **`Rephrase`**     | <code>'REPHRASE'</code>     | Rephrase the input text.                       | 8.2.0 |
...
The rewriting is performed on-device by Gemini Nano. This requires a device that supports Gemini Nano, Android API level 26 or higher and a locked bootloader (see [Requirements](#requirements)). Use the `checkFeatureStatus(...)` method to check whether the feature is available on the device.
...
### Why does `rewrite(...)` return multiple results?

The model may return more than one suggestion. The suggestions are sorted in descending order of confidence, so the first result is the most confident one.

### How do I get the rewritten text as a stream?

Add a listener for the `inferenceProgress` event before calling `rewrite(...)`. The listener is called with each partial result as it is generated. The promise returned by `rewrite(...)` resolves with the full result.
...
See [CHANGELOG.md](https://github.com/capawesome-team/capacitor-mlkit/blob/main/packages/genai-rewriting/CHANGELOG.md).

## License

See [LICENSE](https://github.com/capawesome-team/capacitor-mlkit/blob/main/packages/genai-rewriting/LICENSE).
